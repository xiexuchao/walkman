...
```
  ...
  P(A2|B) = P(A2 ∩ B)/P(B) = (P(B|A2) . P(A2)) / ( P(A1)P(B|A1) + P(A2)P(B|A2) )
  
  根据概率树的信息，带入上面两个公式可以得到P(A1|B) = 0.4262, P(A2|B) = 0.5738
  
  注意这个应用中，随机选择一个部件，是从供应商1提供的概率是65%, 然而当给定这个部件为坏的这个信息后， 从供应商1提供的概率降低到42.62%. 实际上， 组建是坏的，这个概率要比从供应商2提供的要好些。
  
  贝叶斯理论适合那些我们想要计算后验概率的事件是互斥的，并且它们的组合又是整个样本空间的情况。比如有n个互斥事件A1, A2, ..., An, 它们的组合是整个样本空间， 贝叶斯定理可以用于计算任何这里展示的后验概率P(Ai|B). 
  
  贝叶斯定理的公式:
  
  
  因此P(A1|B) = P(A1 ∩ B) / P(B) = ( P(B|A1).P(A1) ) / ( P(A1)P(B|A1) + P(A2)P(B|A2) )
  ![](https://github.com/walkerqiao/walkman/blob/master/docs/da/sfbe_bayes_formula.png)
  
  使用先验概率P(A1), P(A2), ..., P(An), 以及相应的条件概率P(B|A1), P(B|A2), ..., P(B|An), 贝叶斯公式可以计算事件A1,A2,...,An的后验概率。
  
  表格方法
  表格方法在帮助贝叶斯理论计算的时候非常有用。 这种方法展示在表4.7。
  ![](https://github.com/walkerqiao/walkman/blob/master/docs/da/sfbe_bayes_tabular_approach.png)
  
  计算步骤如下:
  1. 准备下面的三列: 列1--后验概率需要的互斥事件序列Ai; 列2--每个事件的先验概率P(Ai); 列3--条件概率，新信息B给定每个事件的概率P(B|Ai)
  2. 在第四列，为每个事件和信息B计算联合概率P(Ai ∩ B), 使用乘法法则得到。这些联合概率是相应的第二列和第三列的乘积。也就是P(Ai ∩ B) = P(B|Ai)P(Ai).
  3. 在第四列计算联合概率的和。 也就是信息的概率P(B). 上面例子表述为，坏部件是由来自A1的坏部件和来自A2的坏部件。
  4. 在第五列，计算后验概率，使用和条件概率的基本关系: P(Ai|B) = P(Ai ∩ B)/P(B). 注意这里的P(Ai ∩ B)在第四列，P(B)就是第四列的和。
  
  注意:
  * 贝叶斯理论在决策分析中广泛使用。 先验概率通常是由决策制定者主观评估的。 采集样本信息，计算后验概率，用于选择最好的决定。
  * 事件和它的补集是互斥的， 并且它们的组合是整个样本空间。 因此，贝叶斯定理总是对计算事件和其补集的后验概率是可用的。
  
### 总结
  本章我们介绍了基本的概率概念以及介绍了概率分析如何为决策指定提供有用信息。 我们描述了概率如何能被解释为事件可能发生的数字度量。
  事件的概率计算方法有两种:
  1. 将事件包含的实验结果集的每个结果的概率进行叠加得到
  2. 使用加法法则、条件概率以及乘法法则，根据关系来计算。
  
  对于那些有额外信息可以获取的情况，我们看到贝叶斯定理如何应用于获取修正的或后验概率。
  
### 词汇
  * 概率(Probability): 一个事件可能发生的数字度量。
  * 实验(Experiment): 产生定义良好的结果的过程。
  * 样本空间(Sample Space): 所有实验结果的集合。
  * 样本点(Sample Point): 样本空间的一个元素。 一个样本点代表一个实验结果。
  * 树图(Tree Diagram): 帮助多步实验视觉化的图形化呈现。
  * 概率分配的基本必要条件(Basic requirements for assigning probabilities):
  * 分类方法(Classical method)
