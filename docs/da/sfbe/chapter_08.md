...
### 8.3 确定样本尺寸
  ...
  这个样本尺寸提供在选择的置信水平上期望的误差幅度。等价描述为“已知置信水平， 希望误差幅度在给定范围内，那么样本尺寸应该是多少?”
  在等式8.3中，E是用户希望接受的误差幅度， z<sub>α/2</sub>直接遵循开发区间评估中使用的置信水平。虽然用户偏好需要考虑，但是95%依然是最常选择的值(z<sub>0.025</sub> = 1.96)
  最后，使用等式8.3需要一个总体标准方差σ的值。然而， 即使σ未知， 我们也可以使用等式8.3为我们提供一个初步的或规划的σ。 在实践中， 可以选择下面三种之一来确定σ:
  1. 使用从之前案例中计算的总体标准方差估计作为σ的规划值。
  2. 使用一个试验研究选择一个初步样本。 从这个初步样本得到的样本标准方差可以用于σ的规划值。
  3. 使用判断或最佳猜测作为σ的值。 例如，我们可能开始通过估计总体中的最大和最小数据值。最大值和最小值之间的差提供了对数据范围的估计。 最后，范围除以4通常建议作为标准方差的粗糙近似， 因此可以作为σ的可接受规划值。
