...
#### 频数分布的图形表示
  销售经理、证券分析师、医院管理者以及其他忙碌于工作的惯例人员常常需要反映销售情况、股票价格或医院费用的趋势的简明图形。这些趋势通常可以用图表或图形来描绘出来。三种能帮助描绘频数分布的图表分别是直方图，频数折线图和累积频数折线图。
  
##### 直方图 
  一种常见的描述频数分布的方法是直方图(histogram).
> 直方图 横轴标示各组， 纵轴标示频数的一种图形。 频数由直条的高度标示，并且直线之间相互紧邻。

  直方图利用一系列相邻的矩形描述频数分布，每一个矩形的高度与改组代表的频数成正比。
  
##### 频数折线图
  在频数折线图中，各组的组中使用线段链接起来。
  
  频数折线图(frequency polygon) 类似于直方图。 它是由连续组中值与组频的交点的线段组成。
  
  直方图和频数折线图都是便于我们对数据的主要特征(高点、低点、集中点等等)得到直观印象。尽管两种图形在用途商是相似的，但是也有不同点。
  直方图的优点是用直条描述每一组， 直线的高度表示每组的频数；
  频数折线图能够直接比较两个或多个频数分布。
  
##### 累积频数分布(cumulative frequency distribution)
  
##### 数据的其他图形表示法
  直方图、频数折线图和累积频数折线图都有很强的视觉吸引力。 也就是说，他们可以抓住读者的注意力。下面我们来考察一下线图、条形图和饼形图。
  
  线图: 显示一个变量在一段时间内的变化。
  条形图(bar chart): 可以用来描述各种测量尺度--列名尺度、定序尺度、定距尺度或定比尺度。
  饼形图(pie chart): 对描述列名尺度的数据特别有用。占比之类的。

#### 错误的图形
  当你创建一个图形时，一定要小心，不要产生误导或错误的表示。无论何时当你看到图表或图形时，都要仔细研究它。要问你自己作者试图展示的是什么? 作者有什么偏见吗?
  
  * 误导一: 缩小Y轴范围， 即将Y轴的刻度间距变小。
  * 误导二: Y轴以非零开始

#### 本章小节
  1. 频数分布就是把一组数据分到互不包含的组里以显示各组中观察值的个数。
  2. 相对频数分布显示了每一组占观察值总数的百分比
  3. 茎叶图是表现一组数据的统计方法
  4. 用图形描述频数分布有两种方法
  5. 累积频数折线图显示了小于某一个确定值的观察值的个数
  6. 报纸和杂志商使用多种图表
  <ul>
    <li>线图是显示一段时间销售或收入的趋势的理想选择</li>
    <li>条形图与线图相似，他对显示以列名尺度表示的数据的变化</li>
    <li>饼图对显示各个组成部分占总体的百分比情况很有用</li>
  </ul>
