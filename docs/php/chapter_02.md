...
```
  你可能会猜测，这里有一个集合函数准换大多数数据类型。 一个值得一提的例外是convert_to_resource(), 那样资源是没有意义的，通过定义，映射到用户空间真正有价值的东西。
  
  如果你担心convert_to_string()的事实，可以不能撤销的改变传入函数的值。在实际代码中， 这将一般是不好的，当然，这不是引擎所做的。下一章，你会看到各种方式使用convert函数来安全改变值的内容在那些不能销毁它们原来内容的情况下。

=========================

### 总结
  本章你看到了PHP变量的内部表现。 学到如何区分类型，设置和检索值， 以及添加变量到符号表中，以及再从其中取值。 下一章，你将基于这些知识学到如何拷贝一个zval, 如何再不再需要的时候销毁它们，以及更加重要的， 如何再不需要的时候避免拷贝。
  
  你也将看看Zend的每个请求的内存管理层，以及检测持久和非持久分配。在下一章结尾你将有一些必要基础，开始创建一个工作扩展以及使用你自己的变化的代码做实验。
