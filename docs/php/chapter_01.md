...
```
  虽然现在不是所有的代码片都能清楚含义，你只需要注意回调函数使用了EXT_G宏，这就是说在线程安全构建下面需要用到tsrm_ls指针。改变函数原型也没有用，因为外部库没有PHP线程安全模型，也不应该如此。那么如何才能让tsrm_ls涵盖到这种方式下面并且可用呢?
  
  解决方法来自于Zend宏叫做TSRMLS_FETCH(). 当把这个宏放在代码片段的顶部，这个宏会执行一个查询，基于当前线程上下文，然后声明一个局部拷贝的tsrm_ls指针。
  
  虽然它会尝试在每个地方使用这个宏并且通过函数调用传入tsrm_ls不被打扰，重要的是注意TSRMLS_FETCH()调用需要大量处理时间完成。当然在单独的迭代中不很明显，但是当你的线程数目增加，你调用TSRMLS_FETCH()的实例数量增加，你的扩展逐渐对这个出现这个瓶颈。一定要有节制的使用它。
> 注意 为了保证和C++编译器兼容，确保将TSRMLS_FETCH()和所有变量声明放到在给定代码块任何语句前面。因为TSRMLS_FETCH()宏本身可以涉及不同方式的对， 最好让这个最后一个变量声明在给定声明头中。
