...
#### 总结
  ...
  复制时分离、引用时分离、写时分离几种分离场景。
  
  多个变量都具有相同的内容，多个变量可同时指向相同内容。 当需要修改其中一个变量内容时，发生写时复制。
  
  当对多个指向同一变量， 这时新增一个变量指向其中一个变量， 这时通过新变量修改变量内容时，发生引用时分离。
  
  当对有引用的变量进行数据拷贝时，发生复制时分离。
  
  所有的分离都是为了解决refcount, is_ref结合不能解决的场景，提出的解决方案。
