...
### 4.19 文件的时间
  ...
  * st_ctime: 文件i节点状态的最后修改时间， 比如chmod, chown命令。 可以使用ls -lc查看。
  ...
  ls(1)命令按这三个时间值中的一个排序进行显示。 按系统默认(用-l或-t选择项调用时),它按文件的修改时间的先后顺序显示。 -u选择项使用其存取时间排序。 -c选择项目使用其更改状态时间排序。
  
  下表给出各种函数对这三个时间的作用。回忆4-14节所说，目录时包含目录项(文件名和相关的i节点编号)的文件，增加、删除或修改目录项会影响到与其所在目录相关的三个时间。这就是在下表包含两个列的原因。 其中一列是与该文件或目录相关的三个时间，另一列是与所引用的文件的父目录相关的三个时间。例如，创建一个新文件影响到包含此新文件的目录， 也影响到该文件的i节点。但是，读或写一个文件只影响到该文件的i节点，而对父目录则无影响.
...
```
  ...
  常见的错误是为文件指定相同的模式: 只有读写权限。 但是目录，我们一般至少需要执行的权位， 允许访问目录中的文件名。
  
  新建目录的用户ID和组ID根据4.6中描述的规则来确定。
  
  mkdirat函数类似mkdir函数。当fd参数指定AT_FDCWD,或者pathname指定绝对路径，mkdirat和mkdir的行为一样。 否则，fd参数是一个打开的目录，相对目录的路径通过它计算。
  
  空目录可以通过rmdir函数删除。 回忆一下，空目录是只包含.和..入口的目录。
```
#include <unistd.h>
int rmdir(const char *pathname);
```
  如果使用这个调用目录的连接变为0， 并且没有其他进程打开该目录，那么该目录占用的空间会被释放掉。 如果一个或多个进程打开这个目录，当连接到达0， 最后一个连接被删除，并且.和..入口也在该函数返回之前被删除。
  
  额外的， 没有新文件可以被创建在该目录中。 目录不被释放， 然而，直到最后一个进程关闭它的时候。

### 4.22 读取目录
  对某个目录具有存取许可权的任一用户都可读该目录，但是只有内核才能写目录(放置文件系统发生混乱)。 前面我们讲到，一个目录的写许可权位和执行权位决定了在该目录中能否创建新文件以及删除文件， 它们并不表示能写目录本身。
  
  目录的实际格式依赖于unix的具体实现。 早期的系统，例如V7, 有一个比较简单的结构: 每个目录项是16字节， 其中14个字节是文件名，2个字节是i节点编号数。 而对于4.2BSD而言，由于它允许相当长的文件名，所以每个目录项的长度是可变的。 这就意味着读目录的程序与系统有关。为了简化这种变化，Unix现在包含了一套与读目录有关的例程，它们是POSIX.1的一部分。
```
#include <sys/types.h>
#include <dirent.h>
...
```
  回忆一下，在程序1.1使用了这些函数。
  定义在头文件<dirent.h>中的dirent结构与实现无关。SVR4, 4.3BSD定义此结构至少包含下列两个成员:
```
struct dirent{
  ino_t d_ino;
...
```
  ...
  DIR结构是一个内部结构，它由这四个函数用来保存正被读的目录的有关信息。其作用类似于FILE结构。 FILE结构由标准I/O库维护。
  
  由opendir返回的指向DIR结构的指针由另外三个函数使用。 opendir执行初始化操作， 使第一个readdir读目录中的第一个目录项。目录中各目录项的顺序与实现有关。 它们通常并不按字母顺序排列。
  
  实例: 我们讲使用这些目录例程编写一个遍历文件层次结构的程序，其目的是得到各种类型的文件数。 系统V提供了一个实际遍历此层次结构的函数ftw(3), 对于每一个文件它都调用一个用户定义函数。此函数的问题是:对于每一个文件，他都调用了stat函数，这就使程序跟随符号连接。 例如，如果从root开始，并且有一个名为/lib的符号连接，它指向/usr/lib, 则所有在/usr/lib中的文件都两次计数。 为了纠正这一点， SVR4提供了另外一个nftw(3)， 它具有一个停止跟随符号连接的选项。 尽管可以使用nftw,但是为了说明目录例程的使用方法，我们还是编写了一个简单的文件遍历程序。
```
#include "apue.h"
#include <sys/types.h>
#include <sys/stat.h>
...
### 4.27 小节
  本章内容围绕stat函数，详细介绍了stat结构中的每一个成员这使得我们对unix文件的各个属性都有所了解。对文件的所有属性以及对文件进行操作的所有函数都有完整的了解对各种Unix程序涉及都非常重要。
