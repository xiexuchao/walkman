...
## 1.11 系统调用和类库
  ...
  系统调用和库函数之间的另一个差别是:系统调用通常提供一种最小界面，而库函数通常提供比较复杂的功能。 我们从sbrk系统调用和malloc库函数之间的差别可以看到这一点, 在以后当比较不带缓存的i/O函数以及标准I/O函数时，还将看到这种差别。
  
  进程控制系统调用(fork, exec,wait)通常由用户的应用程序直接调用。但是为了简化某些常见的情况，Unix系统也提供了一些库函数；例如system和popen。 
  
## 1.12 总结
  本章快速浏览了Unix。 说明了一些后面会用到的一些术语，了解了一些小的Unix程序实例，等等。
